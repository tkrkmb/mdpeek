...
| Cmd+F（macOS）、Ctrl+F（Linux） | ページ内を検索する。Enter／Shift+Enter で次／前の一致へ移り、Esc で閉じる。検索語はNeovimの `/` と同じ書き方（`\<word\>`、`foo\\|bar`、`\v(a\|b)+`、`\zs` など）で書ける。`\c`／`\C` で大文字と小文字の扱いを決め、なければ検索語に大文字が含まれるときだけ区別する。図と数式の中は検索しない |

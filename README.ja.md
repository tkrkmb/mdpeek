...
対象にできるのは、`filetype`が`markdown`でファイル名のある通常のバッファだけです。入力が約200ms止まると表示が更新されます。カーソルのあるブロックが画面外に出たときだけ、そのブロックが画面の上から1/3に来るようにスクロールします。コードブロックの中では、ブロックではなく行に追従します。ウィンドウのタイトルは`MdSight — Linked to Neovim`です。

Neovimで検索すると（`/`、`?`、`*`、`#`）、プレビューでも同じ一致を青で強調し、カーソルのある一致は濃い青にします。`:noh`で消えます。Markdownの記号（`**太字**`など）を含む一致は強調しません。
...
| `j`／`k`、`Ctrl-d`／`Ctrl-u`、`gg`／`G` | 少しずつ、半画面ずつ、先頭／末尾へスクロールする（Neovimのカーソルは動かない） |
| コードブロックの右上のコピーのボタン（マウスを載せると出る） | コードをコピーする |
| `/`、`n`／`N` | ページ内検索を開く。次／前の一致へ移る（検索の帯を閉じていれば、最後の検索語で開き直す） |
| Cmd+F（macOS）、Ctrl+F（Linux） | ページ内を検索する。Enter／Shift+Enter で次／前の一致へ移り、Esc で閉じる。検索語はNeovimの `/` と同じ書き方（`\<word\>`、`foo\\|bar`、`\v(a\|b)+`、`\zs` など）で書ける。`\c`／`\C` で大文字と小文字の扱いを決め、なければ検索語に大文字が含まれるときだけ区別する。図と数式の中は検索しない。探すのに時間のかかるブロック（長い段落での `.*.*.*x` など）は飛ばし、件数に「(partial)」と付ける |
| Cmd+クリック（macOS）、Ctrl+クリック（Linux） | Neovimのカーソルを、クリックしたブロックの開始行へ移す（コードブロックの中では、クリックした行へ） |
...
戻る／進むでは、その文書で読んでいた位置に戻ります。Neovimのプレビューでリンクを辿ると、Neovim側の対象も同じファイルに切り替わります。保存していない変更があるなどでNeovimが開けなかったときは、通知が出てプレビューも元のままです。`:MdSight`で対象を切り替えると、戻る／進むの履歴は空になります。

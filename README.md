...
| Cmd+F (macOS), Ctrl+F (Linux) | Find in the page. Enter / Shift+Enter moves to the next / previous match, Esc closes. Patterns are written as in Neovim's `/` (such as `\<word\>`, `foo\\|bar`, `\v(a\|b)+`, `\zs`); `\c` / `\C` set the case, otherwise case is ignored unless the pattern has an uppercase letter. Diagrams and math are not searched. Blocks that take too long to search with a pattern (such as `.*.*.*x` on a long paragraph) are skipped, and the count shows "(partial)" |

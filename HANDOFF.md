...
段階5「仕上げ」まで完了・コミット済み。計画の5段階はすべて実装した。
`git@github.com:tkrkmb/mdpeek.git` の `main` にプッシュ済み（31コミット）。

## 次にやること

- 利用者から新しい依頼が出たときに着手する。仕様を変える場合は、先に `AGENTS.md` を更新してから実装する
- コミットしたら `git push` まで行う（`origin/main` を追跡している）

## 未解決の問題と、確認できていないこと

- 画面の見た目とスクロール同期の手触りは、利用者の実機確認に依存する（GNOMEがプログラムからのスクリーンショットを拒否する）
- `beforeBuildCommand` は `ui/` で実行される。`npm run build` と書く（`--prefix ui` は誤り）
- 起動直後、登録時に送るカーソル行は、画面の準備が間に合わないと取りこぼす（実害なし）
- mermaid は 11.17.2（12系はlodash-esの勧告があるため下げた）。`npm audit` は0件
- 親プロセスが消えたあとの標準エラー出力はEPIPEでパニックするため、`main.rs` の `report()` 経由で書く
- `cargo tauri build` は `src-tauri/Cargo.toml` の `tauri-build` 行を書き換えることがある（意味は変わらない）

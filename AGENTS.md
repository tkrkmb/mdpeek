...
- 起動引数が `--nvim <socket> --token <token>` ならNeovim連携モード、単一の位置引数（ファイルパス、`--foreground` を付けてもよい）ならスタンドアローンモードとして起動する。それ以外の引数の組み合わせはエラーを表示して終了する
...
- Neovim連携モードで、`rpc.open` 以外の理由で対象世代が変わったら（`:MdPeek` による対象の切り替え）、窓が隠れていれば前面に出す。フォーカスは移さない（Neovimでの入力を妨げない）。macOSでは `NSWindow` の `orderFrontRegardless`、Linuxでは GDK のウィンドウの `raise` を使う。LinuxのWaylandでは前面に出ないことがあり、それは既知の制限としてREADMEに書く
...
- ウィンドウは1枚だけにする。タイトルは、Neovim連携モードでは `MdPeek — Linked to Neovim`、スタンドアローンモードでは `MdPeek — Read Only` にする
